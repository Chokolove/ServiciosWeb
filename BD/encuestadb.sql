...
END$$
 ...
 DELIMITER //
 CREATE PROCEDURE STP_ListarEncuesta()
   BEGIN
   SELECT e.idEncuesta,e.idUsuario, u.nomUsu, e.nomEncuesta, e.fechaCreacion, e.idEstado, te.nomEstado,
   (select count(idPregunta) from pregunta where idEncuesta = e.idEncuesta) as cant_preg
   FROM encuesta e 
